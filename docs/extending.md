...
本文档面向想学习如何扩展本项目的开发者，重点讲扩展思路、常见入口与实践顺序。涉及必须遵守的约束、归口规则和测试基线时，以 `AGENTS-DEV.md` 为准。
...
当你要真正开始改代码，而不是只是理解思路时，请先切换到 `AGENTS-DEV.md`；当你需要更细的专题知识时，再按需读取 `agents_docs/`。尤其在以下场景，不建议只看本文件：
...
`/analyze` 的职责是初始化第一版 Agent 文档体系。

- 如果 `AGENTS-DEV.md` 不存在，命令会生成首版开发主手册，并按项目特征动态补充 `agents_docs/` 专题文档
- 如果该文件已存在，命令应直接停止
- 如果工作区内存在多项目或多模块结构，命令需要先识别模块边界、依赖关系、公共模块职责与启动入口，再写入开发手册
- 如果项目存在明显开发风格偏好，命令应把这类知识显式整理出来；当信息足够丰富时，优先沉淀到 `agents_docs/` 的专题文档
- 对大型仓库，命令应先结构化采集，再按主题分段成稿，最后统一整理索引与交叉引用，而不是把全部写作压到最后一步
