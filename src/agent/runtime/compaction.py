...
- 任务目标是最高优先级恢复信息：必须识别并保留用户最初或最新明确要求解决的问题、成功标准、交付物、范围边界和明确禁止事项；如果历史中目标发生变化，必须说明当前有效目标以及被替换的旧目标。
- 执行计划文件、TODO List、任务清单、checkpoint、未完成项属于最高优先级信息，必须保留路径、状态、优先级、依赖关系和下一步动作。
- 保留当前任务目标、已完成工作、正在修改或需要继续检查的文件、关键技术决策及其原因。
- 保留用户的关键需求、长期偏好、约束条件、安全要求和明确禁止事项。
- 保留重要工具调用结论、验证结果、失败原因、阻塞点和风险；如果工具输出被截断或附件被省略，只记录可见结论、完整输出路径或后续读取建议，不假设完整内容。
""".strip()

    compact_prompt = """
你正在执行一次**上下文检查点压缩**。请基于以上会话历史生成上下文压缩摘要，为另一个将继续此任务的 LLM 创建一份交接摘要。

请严格按下面结构输出，缺失信息可写“未明确”，但不要省略“任务目标”：

## 任务目标
- 用户当前真正要完成的任务是什么。
- 成功标准、预期交付物、范围边界、明确禁止事项。
- 如果目标在历史中发生变化，说明当前有效目标与变化原因。
...
## 计划与 TODO
- 执行计划文件、TODO List、任务清单、checkpoint、未完成项。
- 必须保留 todo_read/todo_write、plan_path、状态、优先级、依赖关系和下一步动作。

## 重要上下文与约束
- 用户偏好、项目规范、安全要求、性能权衡、边界条件。
- 继续工作所需的关键数据、示例或参考信息。

## 验证、风险与阻塞
- 已运行的验证、测试结果、失败原因。
- 仍需验证的内容、风险点、阻塞点和建议的下一步。
